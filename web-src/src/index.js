...
import actionWebInvoke, { getJWTAccessToken } from './utils.js';
...
async function invokeAction (action, _headers, _params) {
  ...
  } else if (!state.imsToken && !headers.authorization) {
    // local / testing mode: get access token from JWT
    const localToken = await getJWTAccessToken()
